...
// ------------------------------------------------------------
// CLASS FOR REPRESENTING A BIT ARRAY
// ------------------------------------------------------------
class BitArray
{
public:
	BitArray();									  // DEFAULT CONSTRUCTOR
	BitArray(int numberOfBitsNeeded);			  // CONSTRUCTOR WITH NUMBER OF BITS
	void setBit(int locationOfBit, int bitValue); // SETTING THE BIT VALUE
	bool getBit(int locationOfBit);				  // GETTING THE BIT VALUE

private:
	int numberOfBitsNeeded; // NUMBER OF BITS NEEDED
	char *bits;				// ARRAY TO STORE THE BITS
};
